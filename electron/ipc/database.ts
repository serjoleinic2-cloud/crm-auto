...
function registerHandlers(): void {
  ...
  ipcMain.handle('orders:delete', (_e, id: number) => {
    ...
    if (order) _writeHistory(order.client_id as number, 'order_delete',
      `Заказ удалён: ${order.brand ?? ''} ${order.model ?? ''}`);
    return true;
  });
  ...
  ipcMain.handle('consent:update', (_e, clientId: number, data: Record<string, unknown>) => {
    const old = db.prepare('SELECT * FROM consent WHERE client_id=?').get(clientId) as Record<string, unknown> | undefined;
    if (!old) {
    ...
    _writeHistory(clientId, 'consent_update',
      `Согласие на обработку ПД: ${statusLabels[data.status as string] ?? data.status}`);
    return true;
  });
  ...
  ipcMain.handle('dashboard:getStats', () => {
    ...
    return {
      activeClients:     (db.prepare("SELECT COUNT(*) as c FROM clients WHERE is_archived=0").get() as { c: number }).c,
      needsAttention:    (db.prepare("SELECT COUNT(*) as c FROM clients WHERE is_archived=0 AND next_action_date IS NOT NULL AND next_action_date < ?").get(now) as { c: number }).c,
      todayTasks:        (db.prepare("SELECT COUNT(*) as c FROM clients WHERE is_archived=0 AND next_action_date=?").get(now) as { c: number }).c,
      carsInTransit:     (db.prepare("SELECT COUNT(DISTINCT o.client_id) as c FROM orders o JOIN clients c ON c.id=o.client_id WHERE c.is_archived=0 AND c.status_id=(SELECT id FROM statuses WHERE name='Автомобиль в пути' LIMIT 1)").get() as { c: number }).c,
      newClientsThisWeek:(db.prepare("SELECT COUNT(*) as c FROM clients WHERE date(created_at)>=?").get(weekAgo) as { c: number }).c,
